...
def plot_data(data, indicators, sync_axis=None):
    ...
    if sync_axis is not None:
        p = figure(x_axis_type="datetime", tools="pan, wheel_zoom,box_soom, reset, save", width=1000, 
                   x_range=sync_axis)
    else:
    ...
    p.segment(df.index, df.High, df.index, df.Low, colour="black")
    p.vbar(df.index[gain], width, df.Open[gain],df.Close[gain], fill_colour="00ff00", line_colour="00ff00")
    p.vbar(df.index[loss], width, df.Open[loss],df.Close[loss], fill_colour="ff0000", line_colour="ff0000")
...
layout = column(stock1_text, stock2_text, date_picker_from, date_picker_from, date_picker_to, 
